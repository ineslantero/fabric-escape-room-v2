...
- **HTTPS/443 access** to Fabric and Microsoft Entra endpoints from the machine running Copilot. V2 does **not** require outbound TCP 1433, `sqlcmd`, or Warehouse SQL endpoint access.
- Local tools expected by `skills-for-fabric` REST workflows, typically Azure CLI and JSON tooling. Keep `skills-for-fabric` current before the workshop.
